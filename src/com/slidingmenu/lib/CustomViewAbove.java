...
//			if (DEBUG) Log.v(TAG, "onInterceptTouch moved to:(" + x + ", " + y + "), diff:(" + xDiff + ", " + yDiff + "), mLastMotionX:" + mLastMotionX);
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (thisTouchAllowed(ev)) {
				...
				if (isMenuOpen() && mViewBehind.menuTouchInQuickReturn(ev.getX(),mContent,mScrollX)) {
					mQuickReturn = true;
				}
			} else {
				mIsUnableToDrag = true;
...
//				if (DEBUG) Log.v(TAG, "onTouch moved to:(" + x + ", " + y + "), diff:(" + xDiff + ", " + yDiff + ")\nmIsBeingDragged:" + mIsBeingDragged + ", mLastMotionX:" + mLastMotionX);
		...
		case MotionEvent.ACTION_UP:
			if (mIsBeingDragged) {
				...
				mActivePointerId = INVALID_POINTER;
				endDrag();
			} else if (mQuickReturn && mViewBehind.menuTouchInQuickReturn( ev.getX(),mContent, mScrollX)) {
				// close the menu
				setCurrentItem(1);
				endDrag();
			}
